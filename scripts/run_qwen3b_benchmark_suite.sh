#!/usr/bin/env bash
#SBATCH --job-name=vllm_qwen3b
#SBATCH --output=vllm_qwen3b_suite.log
#SBATCH -p GPU-shared
#SBATCH --gpus=v100-32:1
#SBATCH --time=04:00:00
# SPDX-License-Identifier: Apache-2.0
#
# Run complete benchmark suite for Qwen 2.5 3B across all workloads.
# This script runs ShareGPT, MS-MARCO, and HumanEval benchmarks sequentially.
#
# Usage:
#   sbatch scripts/run_qwen3b_benchmark_suite.sh
#   # OR locally:
#   bash scripts/run_qwen3b_benchmark_suite.sh
#
...
# Print header
echo -e "${BLUE}========================================"
echo "Qwen 2.5 3B Benchmark Suite"
echo "========================================"
echo "Model:        $MODEL"
...
run_benchmark() {
    ...
    local num_prompts=$3
    local max_tokens=$4
    local output_file="${OUTPUT_DIR}/results_qwen3b_${dataset}_${TIMESTAMP}.json"
...
if [ ${#FAILED_BENCHMARKS[@]} -eq 0 ]; then
    ...
    ls -lh "${OUTPUT_DIR}/results_qwen3b_"*"_${TIMESTAMP}.json"
    echo
...
echo "1. Review results: cat ${OUTPUT_DIR}/results_qwen3b_*_${TIMESTAMP}.json | jq"
echo "2. Generate summary: python scripts/generate_results_summary.py --results-dir $OUTPUT_DIR --output $OUTPUT_DIR/SUMMARY.md"
echo "3. Compare with 1.5B/7B: Compare results across model sizes to show scaling behavior"
